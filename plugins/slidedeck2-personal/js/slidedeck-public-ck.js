...
(function(e) {
  var t = "0.3.4", n = "hasOwnProperty", r = /[\.\/]/, i = "*", s = function() {}, o = function(e, t) {
    ...
    var n = f, r = a, i = Array.prototype.slice.call(arguments, 2), s = l.listeners(e), c = 0, h = !1, d, v = [], g = {}, y = [], b = u, w = [];
    u = e, a = 0;
    for (var E = 0, S = s.length; E < S; E++) "zIndex" in s[E] && (v.push(s[E].zIndex), s[E].zIndex < 0 && (g[s[E].zIndex] = s[E]));
    v.sort(o);
    while (v[c] < 0) {
      d = g[v[c++]], y.push(d.apply(t, i));
      if (a) {
        a = r;
        return y;
      }
    ...
    for (E = 0; E < S; E++) {
      ...
      if ("zIndex" in d) if (d.zIndex == v[c]) {
        ...
        if (a) break;
        do {
          c++, d = g[v[c]], d && y.push(d.apply(t, i));
          if (a) break;
        } while (d);
      } else g[d.zIndex] = d; else {
        y.push(d.apply(t, i));
        if (a) break;
      }
    }
  ...
  var m = function(r, i) {
    ...
    if (r.type == "path") {
      ...
      if (g != "none") {
        ...
        var M = w * (g != "diamond" && g != "oval");
        s ? (y = r._.arrows.startdx * h || 0, b = e.getTotalLength(l.path) - M * h) : (y = M * h, b = e.getTotalLength(l.path) - (r._.arrows.enddx * h || 0)), S = {}, S["marker-" + a] = "url(#" + k + ")";
        if (b || y) S.d = Raphael.getSubpath(l.path, y, b);
        m(f, S), r._.arrows[a + "Path"] = C, r._.arrows[a + "Marker"] = k, r._.arrows[a + "dx"] = M, r._.arrows[a + "Type"] = g, r._.arrows[a + "String"] = i;
      ...
      for (S in v) if (v[t](S) && !v[S]) {
        var _ = e._g.doc.getElementById(S);
    ...
    for (var d in s) if (s[t](d)) {
      ...
      switch (d) {
       ...
       case "fill":
        ...
        var D = e.getRGB(v);
        if (!D.error) delete s.gradient, delete l.gradient, !e.is(l.opacity, "undefined") && e.is(s.opacity, "undefined") && m(a, {
          opacity: l.opacity
          ...
          if ("opacity" in l || "fill-opacity" in l) {
            var P = e._g.doc.getElementById(a.getAttribute("fill").replace(/^url\(#|\)$/g, c));
            if (P) {
              var B = P.getElementsByTagName("stop");
              m(B[B.length - 1], {
       ...
       case "stroke":
        D = e.getRGB(v), a.setAttribute(d, D.hex), d == "stroke" && D[t]("opacity") && m(a, {
       ...
       case "fill-opacity":
        if (l.gradient) {
          P = e._g.doc.getElementById(a.getAttribute("fill").replace(/^url\(#|\)$/g, c)), P && (B = P.getElementsByTagName("stop"), m(B[B.length - 1], {
            "stop-opacity": v
          }));
          break;
        }
       default:
        ...
        var j = d.replace(/(\-.)/g, function(e) {
          return e.substring(1).toUpperCase();
        });
        a.style[j] = v, r._.dirty = 1, a.setAttribute(d, v);
      }
    }
  ...
  var C = function(e, t, r) {
    ...
    if ("clip-rect" in a) {
      ...
      if (D.length == 4) {
        ...
        var H = c.clipRect || e._g.doc.createElement("div"), B = H.style;
        B.clip = e.format("rect({1}px {2}px {3}px {0}px)", D), c.clipRect || (B.position = "absolute", B.top = 0, B.left = 0, B.width = i.paper.width + "px", B.height = i.paper.height + "px", c.parentNode.insertBefore(H, c), H.appendChild(c), c.clipRect = H);
      }
      a["clip-rect"] || c.clipRect && (c.clipRect.style.clip = "auto");
    }
    if (i.textpath) {
      var j = i.textpath.style;
      a.font && (j.font = a.font), a["font-family"] && (j.fontFamily = '"' + a["font-family"].split(",")[0].replace(/^['"]+|['"]+$/g, d) + '"'), a["font-size"] && (j.fontSize = a["font-size"]), a["font-weight"] && (j.fontWeight = a["font-weight"]), a["font-style"] && (j.fontStyle = a["font-style"]);
    }
    ...
    if (a.opacity != null || a["stroke-width"] != null || a.fill != null || a.src != null || a.stroke != null || a["stroke-width"] != null || a["stroke-opacity"] != null || a["fill-opacity"] != null || a["stroke-dasharray"] != null || a["stroke-miterlimit"] != null || a["stroke-linejoin"] != null || a["stroke-linecap"] != null) {
      var F = c.getElementsByTagName(f), q = !1;
      F = F && F[0], !F && (q = F = M(f)), i.type == "image" && a.src && (F.src = a.src), a.fill && (F.on = !0);
      if (F.on == null || a.fill == "none" || a.fill === null) F.on = !1;
      if (F.on && a.fill) {
        var R = n(a.fill).match(e._ISURL);
        if (R) {
          F.parentNode == c && c.removeChild(F), F.rotate = !0, F.src = R[1], F.type = "tile";
          var U = i.getBBox(1);
          F.position = U.x + p + U.y, i._.fillpos = [ U.x, U.y ], e._preload(R[1], function() {
            i._.fillsize = [ this.offsetWidth, this.offsetHeight ];
          });
        } else F.color = e.getRGB(a.fill).hex, F.src = d, F.type = "solid", e.getRGB(a.fill).error && (b.type in {
          circle: 1,
          ellipse: 1
        } || n(a.fill).charAt() != "r") && L(b, a.fill, F) && (h.fill = "none", h.gradient = a.fill, F.rotate = !1);
      }
      if ("fill-opacity" in a || "opacity" in a) {
        var z = ((+h["fill-opacity"] + 1 || 2) - 1) * ((+h.opacity + 1 || 2) - 1) * ((+e.getRGB(a.fill).o + 1 || 2) - 1);
        z = u(o(z, 0), 1), F.opacity = z, F.src && (F.color = "none");
      }
      c.appendChild(F);
      var W = c.getElementsByTagName("stroke") && c.getElementsByTagName("stroke")[0], X = !1;
      !W && (X = W = M("stroke"));
      if (a.stroke && a.stroke != "none" || a["stroke-width"] || a["stroke-opacity"] != null || a["stroke-dasharray"] || a["stroke-miterlimit"] || a["stroke-linejoin"] || a["stroke-linecap"]) W.on = !0;
      (a.stroke == "none" || a.stroke === null || W.on == null || a.stroke == 0 || a["stroke-width"] == 0) && (W.on = !1);
      var $ = e.getRGB(a.stroke);
      W.on && a.stroke && (W.color = $.hex), z = ((+h["stroke-opacity"] + 1 || 2) - 1) * ((+h.opacity + 1 || 2) - 1) * ((+$.o + 1 || 2) - 1);
      var J = (r(a["stroke-width"]) || 1) * .75;
      z = u(o(z, 0), 1), a["stroke-width"] == null && (J = h["stroke-width"]), a["stroke-width"] && (W.weight = J), J && J < 1 && (z *= J) && (W.weight = 1), W.opacity = z, a["stroke-linejoin"] && (W.joinstyle = a["stroke-linejoin"] || "miter"), W.miterlimit = a["stroke-miterlimit"] || 8, a["stroke-linecap"] && (W.endcap = a["stroke-linecap"] == "butt" ? "flat" : a["stroke-linecap"] == "square" ? "square" : "round");
      if (a["stroke-dasharray"]) {
        ...
        W.dashstyle = G[t](a["stroke-dasharray"]) ? G[a["stroke-dasharray"]] : d;
      }
      X && c.appendChild(W);
    }
    if (b.type == "text") {
      b.paper.canvas.style.display = d;
      var Y = b.paper.span, et = 100, tt = h.font && h.font.match(/\d+(?:\.\d*)?(?=px)/);
      v = Y.style, h.font && (v.font = h.font), h["font-family"] && (v.fontFamily = h["font-family"]), h["font-weight"] && (v.fontWeight = h["font-weight"]), h["font-style"] && (v.fontStyle = h["font-style"]), tt = r(h["font-size"] || tt && tt[0]) || 10, v.fontSize = tt * et + "px", b.textpath.string && (Y.innerHTML = n(b.textpath.string).replace(/</g, "&#60;").replace(/&/g, "&#38;").replace(/\n/g, "<br>"));
      var nt = Y.getBoundingClientRect();
      b.W = h.w = (nt.right - nt.left) / et, b.H = h.h = (nt.bottom - nt.top) / et, b.X = h.x, b.Y = h.y + b.H / 2, ("x" in a || "y" in a) && (b.path.v = e.format("m{0},{1}l{2},{1}", s(h.x * w), s(h.y * w), s(h.x * w) + 1));
      var rt = [ "x", "y", "text", "font", "font-family", "font-weight", "font-style", "font-size" ];
      for (var it = 0, st = rt.length; it < st; it++) if (rt[it] in a) {
        b._.dirty = 1;
        break;
      }
